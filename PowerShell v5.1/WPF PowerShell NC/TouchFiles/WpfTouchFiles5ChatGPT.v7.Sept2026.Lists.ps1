...
public class TouchFileItemV20260623 : INotifyPropertyChanged
{
    ...
    string fullPath, baseName, displayPath, pathSubtext, lastModified, size, attributes, comment, localNotes, commentSupportEmoji;
    ...
    public string PathSubtext
    {
        get { return pathSubtext; }
        set { if (pathSubtext != value) { pathSubtext = value; Notify("PathSubtext"); } }
    }
...
if ($footerWrapPanel -and ($footerWrapPanel -is [System.Windows.Controls.Panel])) {
    ...
    if ($btnCopyLastErrorId.PSObject.Properties['IsEnabled']) {
...
function Set-ControlEnabledSafe {
    ...
    try {
        $isEnabledMember = $Control.PSObject.Properties['IsEnabled']
        if (-not $isEnabledMember) {
            Write-AppVerbose -Message "$Context skipped: target type '$($Control.GetType().FullName)' does not expose IsEnabled."
            return
        }

        $Control.IsEnabled = [bool]$Enabled
    }
    catch {
...
$itemPropertyChangedHandler   = [System.ComponentModel.PropertyChangedEventHandler]{
    ...
    elseif ($changeArgs.PropertyName -eq 'LocalNotes' -and -not $script:suppressDirtyTracking) {
        Mark-ListDirty -Reason 'local notes property changed'
    }
}
...
function Set-PathVisibility {
    ...
    if ($fileCardList) {
        $fileCardList.Tag = $HidePaths
    }

    foreach ($item in $fileItems) {
        if (-not $item) { continue }
        $item.DisplayPath = if ($HidePaths) { '' } else { [string]$item.FullPath }
    }

    if ($colAbsolutePath) {
        $colAbsolutePath.Width = if ($HidePaths) { 280 } else { 0 }
    }
    if ($colPath) {
        $colPath.Width = if ($HidePaths) { 220 } else { 330 }
    }
}
...
function Invoke-WithWaitCursor {
    ...
    try {
        ...
        $stopwatch.Stop()
        $remaining = [Math]::Max(0, $MinimumMs - [int]$stopwatch.ElapsedMilliseconds)
        if ($remaining -le 0) {
            [System.Windows.Input.Mouse]::OverrideCursor = $null
        }
        else {
            $releaseTimer = New-Object System.Windows.Threading.DispatcherTimer
            $releaseTimer.Interval = [TimeSpan]::FromMilliseconds($remaining)
...
function Register-ShortcutContextMenu {
    ...
    $miOpenShortcut.Add_Click({
        ...
        if ($selected.Count -eq 1) {
            Invoke-WithWaitCursor -Action { Open-SelectedShortcut -Item $selected[0] }
        }
    })
    ...
    $miRemoveFromList = New-Object System.Windows.Controls.MenuItem
    $miRemoveFromList.Header = 'Remove From List'
    $miRemoveFromList.Add_Click({
        ...
        foreach ($row in @($selected)) {
            if ($row) {
                [void]$fileItems.Remove($row)
                $removed++
            }
        }

        if ($removed -gt 0) {
            Mark-ListDirty -Reason 'removed item(s) from context menu'
            if ($lblStatus) { $lblStatus.Text = "Removed $removed item(s) from the list." }
            Write-SessionOutput -Message "Context menu removed $removed item(s) from the list."
            Update-UiState
        }
    })
    ...
    [void]$contextMenu.Items.Add($miRemoveFromList)

    $contextMenu.Add_Opened({
        $selected = Get-SelectedItems
        $enabled = ($selected.Count -eq 1)
        Set-ControlEnabledSafe -Control $miOpenShortcut -Enabled $enabled -Context 'ContextMenu OpenShortcut'
        Set-ControlEnabledSafe -Control $miOpenLocation -Enabled $enabled -Context 'ContextMenu OpenLocation'
        Set-ControlEnabledSafe -Control $miOpenTargetFolder -Enabled $enabled -Context 'ContextMenu OpenTargetFolder'
        Set-ControlEnabledSafe -Control $miRemoveFromList -Enabled ($selected.Count -ge 1) -Context 'ContextMenu RemoveFromList'
    })
...
function Add-Files {
    ...
    foreach ($path in $Paths) {
        ...
        $item.DisplayPath  = if ($chkCompactMode -and $chkCompactMode.IsChecked -eq $true) { '' } else { $info.FullName }
...
function Refresh-FileEntries {
    ...
    Set-ControlEnabledSafe -Control $btnRefresh -Enabled $false -Context 'Refresh-FileEntries Start'
    Update-UiState
    [System.Windows.Input.Mouse]::OverrideCursor = [System.Windows.Input.Cursors]::Wait
        ...
        try {
            ...
            while ($processedThisTick -lt $batchSize -and $index -lt $total) {
                ...
                try {
                    ...
                    $entry.DisplayPath = if ($chkCompactMode -and $chkCompactMode.IsChecked -eq $true) { '' } else { $info.FullName }
            ...
            if ($index -ge $total) {
                ...
                $script:refreshInProgress = $false
                $script:refreshTimer = $null
                Set-ControlEnabledSafe -Control $btnRefresh -Enabled $true -Context 'Refresh-FileEntries Complete'
                [System.Windows.Input.Mouse]::OverrideCursor = $null
            ...
            $script:refreshInProgress = $false
            $script:refreshTimer = $null
            Set-ControlEnabledSafe -Control $btnRefresh -Enabled $true -Context 'Refresh-FileEntries Error'
            [System.Windows.Input.Mouse]::OverrideCursor = $null
...
if ($fileList) {
    $fileList.Add_MouseDoubleClick({
        ...
        try {
            ...
            if ($selected) {
                Invoke-WithWaitCursor -MinimumMs 1000 -Action { Open-SelectedShortcut -Item $selected }
                $mouseArgs.Handled = $true
            }
        }
        catch {
            Add-ErrorLog -Context 'Double-click open failed (table view)' -ErrorRecord $_
        }
    })
}
...
if ($fileCardList) {
    $fileCardList.Add_MouseDoubleClick({
        ...
        try {
            ...
            Add-ErrorLog -Context 'Double-click open failed (card view)' -ErrorRecord $_
        }
    })
}
