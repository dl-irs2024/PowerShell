...
function Get-SessionGridRows {
	...
	try {
		if (-not (Test-Path -LiteralPath $FolderPath)) {
			Add-DebugLog "Session folder not found: $FolderPath"
			return @()
		}
		...
		Add-DebugLog "Found $($jsonFiles.Count) JSON session files in $FolderPath"
		Add-DebugLog "Excluded machine (current): $ExcludeMachineName"
		...
		foreach ($file in $jsonFiles) {
			try {
				...
				if (-not [string]::IsNullOrWhiteSpace($excludeToken) -and ($machineToken -eq $excludeToken -or $machineShortToken -eq $excludeToken)) {
					Add-DebugLog "  -> Skipping excluded machine: $machineValue"
					continue
				}
				...
				Add-DebugLog "  -> File: $($file.Name) | Machine: $machineValue | User: $([string]$obj.UserId) | Status: $computedStatus | LastActivity: $statusTimestamp"
	...
	<Grid Margin="18">
		...
		<Grid Grid.Row="2" Margin="0,12,0,0">
			<StackPanel Orientation="Horizontal" VerticalAlignment="Center">
				...
				<TextBlock Text="Disconnected" FontSize="16.5" Foreground="#365065" VerticalAlignment="Center"/>
			</StackPanel>
		</Grid>
		...
		<Grid Grid.Row="4" Margin="0,12,0,0">
			...
			<Button Grid.Column="4" Name="BtnMinimize" Content="_" Width="34" Height="30" Margin="0,0,8,0" FontWeight="Bold" ToolTip="Minimize to system tray."/>
			<Button Grid.Column="5" Name="BtnCloseAfterSession" Content="Close After Session" Width="150" Height="30" ToolTip="Close the app after the current session ends."/>
		</Grid>

		<Grid Name="StartupOverlay" Grid.RowSpan="5" Background="#B3000000" Visibility="Visible" Panel.ZIndex="1000">
...
function Update-SessionGridData {
	try {
		Add-DebugLog "======= UPDATE-SESSIONGRIDDATA START ======="
		...
		Add-DebugLog "Got $($realRows.Count) real rows from folder"
		if ($script:isSimulationModeEnabled -and $script:simulatedRows.Count -gt 0) {
			$combinedRows = @($script:simulatedRows + $realRows)
			Add-DebugLog "Simulation enabled - combined with $($script:simulatedRows.Count) simulated rows = $($combinedRows.Count) total"
		}
		else {
			$combinedRows = $realRows
			Add-DebugLog "Simulation disabled - using real rows only"
		}

		$combinedRows = @($combinedRows | Sort-Object -Property LastActivity -Descending)
		Add-DebugLog "Final grid will display $($combinedRows.Count) entries:"
		foreach ($row in $combinedRows) {
			Add-DebugLog "  - $($row.MachineName) | $($row.UserId) | Status: $($row.RowStatus) | Activity: $($row.LastActivity)"
		}

		if ($gridSessionFiles -and $combinedRows) {
			$gridSessionFiles.ItemsSource = $null
			$gridSessionFiles.ItemsSource = $combinedRows
			Add-DebugLog "======= UPDATE-SESSIONGRIDDATA END ======="
			...
			Add-DebugLog "ERROR: Grid control or data is null"
		}
	}
	catch {
		Add-DebugLog "ERROR in Update-SessionGridData: $($_.Exception.Message)"
		Add-DebugLog "Stack: $($_.ScriptStackTrace)"
	}
}
...
$refreshAction = {
	try {
		...
		Add-DebugLog "ERROR in refreshAction: $($_.Exception.Message)"
		Add-DebugLog "Stack: $($_.ScriptStackTrace)"
	}
}
...
function Show-MiniSessionPanel {
	...
	# Auto-hide mini window after 4 seconds (non-blocking fade)
	...
	$miniAutoHideTimer.Add_Tick({
		...
		if ($miniWindow.IsVisible) {
			# Non-blocking fade animation using timer
			...
			$fadeTimer.Add_Tick({
				...
				if ($script:fadeOpacity -lt 0) {
					$script:fadeOpacity = 0
				}
...
$btnLog.Add_Click({
	...
	$grid.Background = New-Object System.Windows.Media.SolidColorBrush ([System.Windows.Media.Color]::FromRgb(240, 240, 240))
	...
	$rowDef2.Height = [System.Windows.GridLength]::new(50, [System.Windows.GridUnitType]::Pixel)
	...
	$textBox.FontSize = 11
	$textBox.Foreground = New-Object System.Windows.Media.SolidColorBrush ([System.Windows.Media.Color]::FromRgb(0, 0, 0))
	$textBox.Background = New-Object System.Windows.Media.SolidColorBrush ([System.Windows.Media.Color]::FromRgb(255, 255, 255))
	...
	$btnPanel.Margin = New-Object System.Windows.Thickness(10)
	[System.Windows.Controls.Grid]::SetRow($btnPanel, 1)
	$grid.Children.Add($btnPanel)

	$btnClearLog = New-Object System.Windows.Controls.Button
	$btnClearLog.Content = "Clear Log"
	$btnClearLog.Width = 100
	$btnClearLog.Height = 30
	$btnClearLog.Margin = New-Object System.Windows.Thickness(0, 0, 10, 0)
	$btnClearLog.Add_Click({
		$script:debugLog = @()
		$textBox.Text = ""
		Add-DebugLog "Log cleared"
		$textBox.Text = [string]::Join("`r`n", $script:debugLog)
	})
	$btnPanel.Children.Add($btnClearLog)
...
$window.Add_StateChanged({
	Append-StartupTrace -Path $SettingsPath -EventName 'Main.StateChanged' -Detail ("State={0}" -f $window.WindowState)
})
