				...
				<Grid Grid.Row="2" Margin="4,8,4,4">
					...
					<DataGrid Name="GridSessionFiles"
						...
						<DataGrid.Columns>
							...
							<DataGridTextColumn Header="Last Activity" Binding="{Binding LastActivity}" Width="*"/>
						</DataGrid.Columns>
					</DataGrid>
				</Grid>
			</Grid>
...
function Convert-MainViewToPsObjectText {
	...
	foreach ($row in @($GridRows)) {
		...
		$lines += "        [pscustomobject]@{ MachineName = '{0}'; UserId = '{1}'; UserDisplayName = '{2}'; LastLogin = '{3}'; LastActivity = '{4}' }" -f $machine, $userId, $displayName, $lastLogin, $lastActivity
	}
...
$refreshAction = {
	...
	$gridSessionFiles.ItemsSource = @(Get-SessionGridRows -FolderPath $script:effectiveOutputFolder -TrackedMachines $script:trackedMachines -ExcludeMachineName $machineName)
}
