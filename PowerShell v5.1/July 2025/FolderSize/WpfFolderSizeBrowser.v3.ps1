...
if ($appSettings) {
    ...
    if ($appSettings.window) {
        ...
        # Clamp window position to visible monitor bounds to prevent off-screen windows.
        ...
        $virtualScreenRight = $virtualScreenLeft + $virtualScreenWidth
        $virtualScreenBottom = $virtualScreenTop + $virtualScreenHeight

        if ($window.Left -lt $virtualScreenLeft) {
            $window.Left = $virtualScreenLeft
        }
        if ($window.Top -lt $virtualScreenTop) {
            $window.Top = $virtualScreenTop
        }

        $windowRight = $window.Left + $window.Width
        if ($windowRight -gt $virtualScreenRight) {
            $window.Left = [math]::Max($virtualScreenLeft, $virtualScreenRight - $window.Width)
        }

        $windowBottom = $window.Top + $window.Height
        if ($windowBottom -gt $virtualScreenBottom) {
            $window.Top = [math]::Max($virtualScreenTop, $virtualScreenBottom - $window.Height)
        }
...
function Get-DiskHealthLookupByNumber {
    $lookup = @{}

    $getDiskCommand = Get-Command -Name 'Get-Disk' -ErrorAction SilentlyContinue
    if ($getDiskCommand) {
        try {
            foreach ($disk in (Get-Disk -ErrorAction Stop)) {
                $healthText = [string]$disk.HealthStatus
                if ([string]::IsNullOrWhiteSpace($healthText)) {
                    $healthText = [string]$disk.OperationalStatus
                }
                if (-not [string]::IsNullOrWhiteSpace($healthText)) {
                    $lookup[[int]$disk.Number] = $healthText
                }
            }
        }
        catch {
            # Continue with best-effort disk health fallbacks when Storage cmdlets are unavailable.
        }
    }
...
function Get-DiskSummaryRows {
    ...
    $diskSummaryRows = @()
    $seenDiskIndexes = @{}
    $healthLookup = Get-DiskHealthLookupByNumber
    ...
    foreach ($path in $candidatePaths) {
        ...
        $driveRoot = [System.IO.Path]::GetPathRoot([string]$resolvedPath)
        if ([string]::IsNullOrWhiteSpace($driveRoot)) {
            continue
        }

        $driveLetter = $driveRoot.TrimEnd([char[]]'\\/')
        ...
        try {
            ...
            $diskPartition = Get-CimAssociatedInstance -InputObject $logicalDisk -ResultClassName Win32_DiskPartition -ErrorAction Stop | Select-Object -First 1
            if (-not $diskPartition) {
                continue
            }

            $diskDrive = Get-CimAssociatedInstance -InputObject $diskPartition -ResultClassName Win32_DiskDrive -ErrorAction Stop | Select-Object -First 1
            if (-not $diskDrive) {
                continue
            }

            $diskIndex = [int]$diskDrive.Index
            if ($seenDiskIndexes.ContainsKey($diskIndex)) {
                continue
            }
            $seenDiskIndexes[$diskIndex] = $true

            $sizeBytes = [double]$diskDrive.Size
            $bytesPerSector = [int]$diskDrive.BytesPerSector
            $totalSectors = if ($bytesPerSector -gt 0 -and $sizeBytes -gt 0) {
                [math]::Round($sizeBytes / $bytesPerSector, 0)
            }
            else {
                0
            }
            ...
            if ($healthLookup.ContainsKey($diskIndex)) {
                $healthStatus = [string]$healthLookup[$diskIndex]
            }

            $diskSummaryRows += [PSCustomObject]@{
                DiskNumber = $diskIndex
                Volume = $driveLetter
                BrandName = [string]$diskDrive.Model
                SizeBytes = [double]$sizeBytes
                SizeGB = [math]::Round($sizeBytes / 1GB, 2)
                BytesPerSector = $bytesPerSector
                TotalSectors = [double]$totalSectors
                Health = $healthStatus
            }
        }
        catch {
            # Keep export resilient when querying disk metadata fails for a path.
        }
    }

    return @($diskSummaryRows | Sort-Object DiskNumber)
}
...
function Export-FolderReportXlsx {
    ...
    try {
        ...
        $worksheet.Cells.Item(4, 1).Value2 = "Outline View"
        $worksheet.Cells.Item(4, 2).Value2 = "Collapsed to Level 2"
        $worksheet.Range("A1:A4").Font.Bold = $true
        ...
        foreach ($row in $Rows) {
            ...
            $outlineLevel = [math]::Min([int]$row.Level + 1, 8)
            $worksheet.Rows.Item($rowIndex).OutlineLevel = $outlineLevel
        ...
        try {
            $worksheet.Outline.ShowLevels(2, 1)
        }
        catch {
            try {
                ...
                # Ignore outline collapse failures and preserve exported data.
            }
        }

        $diskSummaryRows = Get-DiskSummaryRows -Paths $SourcePaths
        $diskWorksheet = $workbook.Worksheets.Add()
        $diskWorksheet.Name = "Disk Summary"
        $diskWorksheet.Cells.Item(1, 1).Value2 = "Generated"
        $diskWorksheet.Cells.Item(1, 2).Value2 = (Get-Date -Format 'ddd yyyy-MM-dd HH:mm:ss')
        $diskWorksheet.Cells.Item(2, 1).Value2 = "Scope"
        $diskWorksheet.Cells.Item(2, 2).Value2 = [string]$RootPath
        $diskWorksheet.Range("A1:A2").Font.Bold = $true

        $diskHeaderRow = 4
        $diskHeaders = @('Disk #', 'Volume', 'Brand Name', 'Size GB', 'Size Bytes', 'Bytes / Sector', 'Total Sectors', 'Health')
        for ($diskCol = 0; $diskCol -lt $diskHeaders.Count; $diskCol++) {
            $diskWorksheet.Cells.Item($diskHeaderRow, $diskCol + 1).Value2 = $diskHeaders[$diskCol]
        }

        $diskWorksheet.Cells.Item($diskHeaderRow, 1).EntireRow.Font.Bold = $true
        $diskWorksheet.Cells.Item($diskHeaderRow, 1).EntireRow.Interior.ColorIndex = 15

        $diskRowIndex = $diskHeaderRow + 1
        foreach ($diskRow in $diskSummaryRows) {
            $diskWorksheet.Cells.Item($diskRowIndex, 1).Value2 = [int]$diskRow.DiskNumber
            $diskWorksheet.Cells.Item($diskRowIndex, 2).Value2 = [string]$diskRow.Volume
            $diskWorksheet.Cells.Item($diskRowIndex, 3).Value2 = [string]$diskRow.BrandName
            $diskWorksheet.Cells.Item($diskRowIndex, 4).Value2 = [double]$diskRow.SizeGB
            $diskWorksheet.Cells.Item($diskRowIndex, 5).Value2 = [double]$diskRow.SizeBytes
            $diskWorksheet.Cells.Item($diskRowIndex, 6).Value2 = [int]$diskRow.BytesPerSector
            $diskWorksheet.Cells.Item($diskRowIndex, 7).Value2 = [double]$diskRow.TotalSectors
            
            $healthCell = $diskWorksheet.Cells.Item($diskRowIndex, 8)
            $healthCell.Value2 = [string]$diskRow.Health
            
            $healthStatus = [string]$diskRow.Health
            switch -Wildcard ($healthStatus) {
                "Healthy" {
                    $healthCell.Interior.Color = 0x00B050
                    $healthCell.Font.Color = 0xFFFFFF
                    $healthCell.Font.Bold = $true
                }
                "Warning*" {
                    $healthCell.Interior.Color = 0xFFC000
                    $healthCell.Font.Color = 0x000000
                    $healthCell.Font.Bold = $true
                }
                "Unhealthy*" {
                    $healthCell.Interior.Color = 0xFF0000
                    $healthCell.Font.Color = 0xFFFFFF
                    $healthCell.Font.Bold = $true
                }
                "Unknown" {
                    $healthCell.Interior.Color = 0xBBBBBB
                    $healthCell.Font.Color = 0x000000
                }
            }
        ...
        $diskLastDataRow = [math]::Max($diskSummaryRows.Count + $diskHeaderRow, $diskHeaderRow + 1)
        $diskUsedRange = $diskWorksheet.Range("A$diskHeaderRow", "H$diskLastDataRow")
        $diskUsedRange.Borders.LineStyle = 1
        $diskUsedRange.AutoFilter() | Out-Null
        $diskWorksheet.Columns.Item(4).NumberFormat = "#,##0.00"
        $diskWorksheet.Columns.Item(5).NumberFormat = "#,##0"
        $diskWorksheet.Columns.Item(6).NumberFormat = "#,##0"
        $diskWorksheet.Columns.Item(7).NumberFormat = "#,##0"
        $diskWorksheet.Columns.AutoFit() | Out-Null
        ...
        if ($diskWorksheet) {
            [void][System.Runtime.InteropServices.Marshal]::ReleaseComObject($diskWorksheet)
        }
    ...
    try {
        ...
        switch ($selectedFormat) {
            "XLSX" {
                ...
                Export-FolderReportXlsx -Rows $reportRows -OutputPath $saveDialog.FileName -RootPath $rootLabel -RecurseEnabled $recurseForExport -IncludeAttributeNames $includeAttributeNames -SourcePaths $exportRootPaths
            }
            "Markdown" {
                $rootLabel = if ($exportRootPaths.Count -gt 1) {
                    "Multiple Folders ($($exportRootPaths.Count))"
